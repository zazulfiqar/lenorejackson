...
{{--                        @if(Auth::user()->is_vendor == 0 && Auth::user()->role == 'user')--}}
{{--                            <button type="button" id="sendRequest" class="btn btn-success btn-sm">Request As Vendor</button>--}}
{{--                        @endif--}}
{{--                        @if(Auth::user()->is_vendor == 1 && Auth::user()->role == 'user')--}}
{{--                            <button type="button" id="cancelRequest" class="btn btn-success btn-sm">Cancel Request</button>--}}
{{--                        @endif--}}
                    </form>
                </div>
            </div>
    ...
    jQuery(document).ready(function(){
        ...
        jQuery('#sendRequest').click(function(e){
            ...
            jQuery.ajax({
                ...
                success: function(result){
                    setTimeout(function(){
                        alert('Request Sent to become Vendor...'),1000
                    });
                    location.reload()
                }});
            Cache.delete();
        ...
        jQuery('#cancelRequest').click(function(e){
            ...
            jQuery.ajax({
                url: "{{ route('user-cancel-request') }}",
                method: 'GET',
                cache: false,
                success: function(result){
                    setTimeout(function(){
                        alert('Request Cancelled'),1000
                    });
                    location.reload()
                }});
            Cache.delete()
...
@push('scripts')
    ...
    {{--            console.log('check');--}}
    {{--            alert('Request for vendor work is in Progress');--}}
    {{--        });--}}
    {{--    });--}}
    {{--</script>--}}
    <script>
